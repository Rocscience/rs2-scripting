...
class TestDisplacementDependent(unittest.TestCase):
    ...
    def testDisplacementDependentStageFactors(self):
        self.joint.setSlipCriterion(JointTypes.JOINT_DISPLACEMENT_DEPENDENT)
        stageFactor = self.joint.DisplacementDependent.getStageFactors()[0]
        ...
        stageFactor.setGroundwaterPressureFactor(3215.6)
        self.model.save()
        self.model.close()
        self.model = self.modeler.openFile(self.copiedModelPath)
        self.joint = self.model.getAllJointProperties()[0]
        ...
        self.assertEqual(stageFactor.getGroundwaterPressureFactor(), 3215.6)
