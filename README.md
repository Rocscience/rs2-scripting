...
3. Follow the Build section to build the project to include all the new updates.
4. To generate the documentation run the following:  
...
Model and interpreter pictures in the documentation are saved under docs/pictures folder. All pictures are linked to docstrings in python files. Rocscience logo is saved under _static folder.

Under docs/example_code folder, runAllExamples.bat file can run through all models and generate example result .txt files which will be linked in the example .rst files to show the most updated example results.
