...
class Custom(PropertyProxy):
	...
	def setCustomStiffnessLoadingTable(self, table: list[tuple[float,float,float]]):
		"""
		...
		"""
		return self._callFunction("setCustomStiffnessLoadingTable", [table])
	...
	def setCustomStiffnessUnloadingTable(self, table: list[tuple[float,float,float]]):
		"""
		Tuple element order: (Custom Mode Parameter (p,q,S3...), Young's modulus, Poisson's Ratio).
		Poisson's ratio set to zero and ignored if useContantPoissonsRatio is set to True.
		"""
		return self._callFunction("setCustomStiffnessUnloadingTable", [table])
